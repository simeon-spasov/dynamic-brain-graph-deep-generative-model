...
def _compute_dynamic_fc(X, window_size=30, window_stride=10, measure="correlation", top_percent=5, 
                        max_time=490, zscore=True, self_loops=False, as_graph=True):
...
def data_loader(dataset, batch_size=10):
    ...
    batch_sample_idx = np.array_split(sample_idx, num_samples // batch_size)
