...
class EvolveGraph(nn.Module):
    ...
    def __init__(self, num_samples, num_nodes, embedding_dim, categorical_dim,
                 gamma, sigma, device):
        ...
        self.beta_mean = nn.Linear(self.embedding_dim, embedding_dim).to(device)
        self.beta_std = nn.Sequential(
            nn.Linear(self.embedding_dim, embedding_dim),
            nn.Softplus()).to(device)

        self.phi_mean = nn.Linear(self.embedding_dim,
                                          self.embedding_dim).to(device)
        self.phi_std = nn.Sequential(
            nn.Linear(self.embedding_dim, self.embedding_dim),
            nn.Softplus()).to(device)
        self.nn_pi = nn.Linear(self.embedding_dim, self.categorical_dim).to(
            device
        )  #Model evolution of natural parameters = always positive and sum to 1

        self.rnn_nodes = nn.GRU(2*self.embedding_dim,
                                self.embedding_dim,
                                num_layers=1,
                                bias=True).to(device)
        self.rnn_comms = nn.GRU(2*self.embedding_dim,
                                self.embedding_dim,
                                num_layers=1,
                                bias=True).to(device)
        ...
        self.subject_to_phi = nn.Linear(embedding_dim, self.num_nodes*self.embedding_dim).to(device)
        self.subject_to_beta =  nn.Linear(embedding_dim, self.categorical_dim*self.embedding_dim).to(device)

        
        self.alpha_mean_prior = torch.zeros((embedding_dim)).to(device)
        self.alpha_std_scalar = 1.

        self.decoder = nn.Sequential(nn.Linear(embedding_dim,
                                               num_nodes),).to(device)
    ...
    def forward(self, batch_graphs):
        ...
        for subject in batch_graphs:
            ...
            subject_loss = self.subject_forward(subject_idx, subject_graphs)
            loss+=subject_loss
        ...
        return loss.item()/ len(batch_graphs)
    
    def subject_forward(self, subject_idx, graphs, temp=1.):
        
        loss = 0
        #train_edges_all_graphs = []
        # iterate over list of graphs for a single subject
        #for graph in graphs:
            # get edge indexes
        #    train_edges = [(u, v) for u, v in graph.edges()]
        #    if self.training:
        #        np.random.shuffle(train_edges)
            # edge_batch = np.array_split(train_edges, self.batches)
        #    train_edges_all_graphs.append(train_edges)
        ...
        for graph in graphs:
            ...
            beta_sample = self._reparameterized_sample(
                beta_mean_t, beta_std_t)
            ...
            KLD_beta = self._kld_gauss(beta_mean_t, beta_std_t,
                                       beta_prior_mean, self.gamma)
            KLD_phi = self._kld_gauss(phi_mean_t, phi_std_t,
                                      phi_prior_mean, self.sigma)
            ...
            #print('KLD_z loss is ', KLD_z)
            #These variables store beta^t, phi^t. Required as we update pi^t at every iteration,
            #as well as node node and community embeddings for GRU hidden state and priors over the embeddings
...
def my_softmax_gumbel(logits, device, tau=1, hard=False, eps=1e-10, dim=-1):
    shape = logits.size()
