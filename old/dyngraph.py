...
class EvolveGraph(nn.Module):
    ...
    def __init__(self, num_samples, num_nodes, embedding_dim, categorical_dim,
        ...
        self.beta_std = nn.Sequential(nn.Linear(self.embedding_dim, embedding_dim), nn.Softplus()).to(device)

        self.phi_mean = nn.Linear(self.embedding_dim, self.embedding_dim).to(device)
        self.phi_std = nn.Sequential(nn.Linear(self.embedding_dim, self.embedding_dim), nn.Softplus()).to(device)
        
        self.nn_pi = nn.Linear(self.embedding_dim, self.categorical_dim).to(device)  #Model evolution of natural parameters = always positive and sum to 1

        self.rnn_nodes = nn.GRU(2*self.embedding_dim, self.embedding_dim, num_layers=1, bias=True).to(device)
        self.rnn_comms = nn.GRU(2*self.embedding_dim, self.embedding_dim, num_layers=1, bias=True).to(device)
        ...
        self.decoder = nn.Sequential(nn.Linear(embedding_dim, num_nodes),).to(device)
        self.optimizer = optim.Adam(self.parameters(), lr=0.005, weight_decay=0.)
        self.init_emb()
